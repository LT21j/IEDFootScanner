...
- (void)viewDidLoad
{
    ...
    _tView.layer.cornerRadius = 10;

    rfduinoManager = [RFduinoManager sharedRFduinoManager];
    rfduinoManager.delegate = self;
    
    int numberOfLines = 3;
    self.tView.rowHeight = (44.0 + (numberOfLines - 1) * 19.0);
    ...
    [self.tView.layer insertSublayer:gradient atIndex:0];
...
- (void)didLoadServiceRFduino:(RFduino *)rfduino
{
   // AppViewController *viewController = [[AppViewController alloc] init];
    _homeVC.rfduino = rfduino;
    rfduino.delegate = _homeVC;
    loadService = true;
...
- (void)showAnimate
{
    ...
    [UIView animateWithDuration:.4 animations:^
     {
         [_popupView setTransform: CGAffineTransformMakeScale(1, 1) ];
     }];
}
...
- (void)showInView:(UIView *)theView animated:(BOOL)animated
{
